...
LANEWIDTH = 3.5  # highway lane width in US: 3.7 meters
input_scale = 4
...
# Source points (from actual image)
x = [164, 1190, 825, 555]
y = [719, 719, 305, 305]
...
def window_search(left_fit, right_fit, binary_warped, margin=100, visualization=False):
    ...
    if len(leftx) == 0 or len(rightx) == 0:
        print("[INFO] No lane pixels found after searching. Returning None.")
        return None, None  # Return None if no lane pixels are found
...
def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, binary_sub, threshold=0.6):
    ...
    whole_frame[
        220 + int(h / 2) : 220 + int(2 * h / 2),
        20 : 20 + int(w / 2),
        :
    ] = undist_birdview

    # ✅ Updated all slice indices to use int()
    start_y = 220 + int(h / 2)
    end_y = 220 + int(h)
    start_x = 40 + int(w / 2)
    end_x = 40 + w

    binary_sub_resized = cv2.resize((binary_sub * 255).astype(np.uint8), (0, 0), fx=1/2, fy=1/2)

    whole_frame[start_y:end_y, start_x:end_x, 0] = binary_sub_resized
    whole_frame[start_y:end_y, start_x:end_x, 1] = binary_sub_resized
...
def tracker(binary_sub, ploty, visualization=False):
    ...
    if left_fit is None or right_fit is None:
        ...
        # Optionally, handle the case by reverting to the previous fit
